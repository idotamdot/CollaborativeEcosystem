  ...
  const handleFilterChange = (filters: any) => {
    ...
    if (potentialMatches) {
      ...
      if (locations && locations.length > 0) {
        filtered = filtered.filter(match =>
          locations.some((loc: string) => match.location.toLowerCase().includes(loc))
        );
      }
      ...
      if (skills && skills.length > 0) {
        filtered = filtered.filter(match =>
          match.skills.some((skill: string) => 
            skills.some((s: string) => skill.toLowerCase().includes(s))
          )
        );
      }
      ...
      if (resources && resources.length > 0) {
        filtered = filtered.filter(match =>
          match.resources.some((resource: string) => 
            resources.some((r: string) => resource.toLowerCase().includes(r))
          )
        );
      }
      ...
      if (values && values.length > 0) {
        filtered = filtered.filter(match =>
          match.values.some((value: string) => 
            values.some((v: string) => value.toLowerCase().includes(v))
          )
        );
      }
    ...
    <div className="bg-gradient-to-r from-primary to-primary-dark min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-start mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">Discover Collaborators</h1>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-white/10 backdrop-blur-sm p-6 rounded-lg">
          <TabsList className="mb-6 bg-white/20">
            <TabsTrigger value="matches" className="data-[state=active]:bg-white data-[state=active]:text-primary">
              <Users className="h-4 w-4 mr-2" /> Potential Matches
            </TabsTrigger>
            <TabsTrigger value="invitations" className="data-[state=active]:bg-white data-[state=active]:text-primary">
              <UserPlus className="h-4 w-4 mr-2" /> Sent Invitations
            </TabsTrigger>
            <TabsTrigger value="connections" className="data-[state=active]:bg-white data-[state=active]:text-primary">
            ...
            <div className="flex-1">
                    ...
                    {filteredMatches.length > 0 ? (
                      ...
                      <Card className="bg-white/90 shadow-lg backdrop-blur-sm">
                        <CardContent className="py-12 text-center">
                          ...
                          <h3 className="text-xl font-medium text-primary mb-2">No matches found</h3>
              ...
              <TabsContent value="invitations" className="m-0">
                <Card className="eco-card">
                  <CardHeader>
                    <CardTitle className="text-primary">Sent Invitations</CardTitle>
                    <CardDescription>
                    ...
                    {invitedUsers.length > 0 ? (
                      <div className="space-y-4">
                        {potentialMatches
                          ...
                          .map(user => (
                            <div key={user.id} className="flex items-center justify-between p-3 bg-white/50 rounded-lg eco-accent-border">
                              ...
                              <div className="text-sm text-amber-600 font-medium">Invitation Sent</div>
                            </div>
                          ))
                        }
                      </div>
                      ...
                      <div className="text-center py-8">
                        <UserPlus className="h-12 w-12 text-amber-400 mx-auto mb-4" />
                        <h3 className="text-lg font-medium text-primary">No invitations sent yet</h3>
                        <p className="text-neutral-600 mb-4">Start connecting with potential collaborators</p>
                        <Button 
                          className="bg-amber-300 text-primary hover:bg-amber-200"
              ...
              <TabsContent value="connections" className="m-0">
                <Card className="eco-card">
                  <CardHeader>
                    <CardTitle className="text-primary">Your Connections</CardTitle>
                    <CardDescription>
                  ...
                  <CardContent>
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-medium text-primary">No connections yet</h3>
